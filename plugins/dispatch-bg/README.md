...
The command is a one-liner. The failures are silent. A background session dispatched from the wrong current directory quietly does the wrong repo's work; a botched dispatch leaves a session sitting with no prompt; and the dispatch banner prints success for both. The banner then suggests `claude logs <id>` / `claude attach` / `claude stop` — none of which are real subcommands (verified on Claude Code 2.1.x). The CLI parses them as a fresh prompt, so `claude logs <id>` spins up a new session that answers the word "logs", which reads exactly like a broken agent. The only job subcommand is `claude agents`, and it takes only `--json` / `--all` / `--cwd` plus defaults for the agent view — there is no scripted stop, attach, or reply.
