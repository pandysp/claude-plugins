#!/usr/bin/env bash
# Route Claude's worktree creation through worktrunk. Fail loud on any problem.
set -euo pipefail

INPUT=$(cat)
{
  IFS= read -r -d '' name
  IFS= read -r -d '' target_cwd
} < <(printf '%s' "$INPUT" | python3 -c '
import sys, json
d = json.load(sys.stdin)
sys.stdout.write((d.get("name") or "claude-" + d["session_id"][:8]) + "\0")
sys.stdout.write(d["cwd"] + "\0")
')
cd "$target_cwd"

# `if !` form is required so set -e does not swallow the failure before we can report it.
if ! output=$(wt switch -c "$name" -y 2>&1); then
  printf 'wt switch -c failed. Output:\n%s\n' "$output" >&2
  exit 1
fi

path=$(printf '%s' "$output" | sed -nE 's|.*worktree @ (.+)$|\1|p' | head -1)
path="${path/#\~/$HOME}"

if [[ -z "$path" || ! -d "$path" ]]; then
  printf 'wt switch -c produced no usable path. Output:\n%s\n' "$output" >&2
  exit 1
fi

echo "$path"
