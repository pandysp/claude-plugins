...
name: verify-claims
description: >-
  Identify and verify unverified claims in your own responses. Especially LOAD-BEARING ones: claims about to drive a design decision, commit, diagnosis, or user-visible recommendation. Self-invoke proactively when a claim is about to become the foundation for action. Also trigger when you catch yourself using hedging language ("likely", "probably", "presumably", "appears to", "seems like"). That hedging is a signal you're uncertain but might present the claim anyway. Trigger when the user invokes the verify-claims skill or says "verify claims", "prove it", "back that up", or "check your claims".
---
